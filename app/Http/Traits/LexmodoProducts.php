...
use App\Helpers\ClientsBuilder;
...
trait ApplicationProducts
{
    use AccessToken, gRPCErrorLog;

    public  function getApplicationProducts(User $user, int $last_updated = 1, int $display_result = 50)
    {
        ...
        while (true) {
            ...
            if ($this->checkGrpcErrors($product_arr, $user->store_id) == false) return $this->checkGrpcErrors($product_arr, $user->store_id);

            if (($counter >= 1) || ($product_arr[0]->getMessage() == 'no products available')) {
                yield $products_array;
                if ($counter == 0) {
                    // $this->log(['message' => 'There\'s no products available'], $this->store_id);
                    break;
            ...
            // dd($product_result_response->serializeToJsonString());
            ...
            // dd($products[0]->serializeToJsonString());
    ...
    public function get_all_Application_products(int $storeId, int $lastSynced = 1, int $display_result = 50, $rule_query = null)
    {
        $product_client = ClientsBuilder::getProductsClient();
        ...
        while (true) {
            ...
            if ($this->checkGrpcErrors($product_arr, $storeId) == false) {
            ...
            if (($counter >= 1) || ($product_arr[0]->getMessage() == 'no products available')) {
                yield   $products_array;
                // dd($product_arr[0]->serializeToJsonString());
                if ($counter == 0) {
                    // $this->log(['message' => 'There\'s no products available'], 0);
                    // dump($product_arr[0]->serializeToJsonString());
                    break;
                } elseif ($product_arr[0]->getMessage() == 'no products available') {
                    // dump($product_arr[0]->serializeToJsonString());
                    break;
    ...
    public function get_all_Application_variants(int $storeId, int $last_updated = 1, int $display_result = 50)
    {
        $product_client = ClientsBuilder::getProductsClient();
        ...
        while (true) {
            ...
            if ($this->checkGrpcErrors($product_variants, $storeId) == false) {
                dump('HHHHHHHHHHHHHHEEEEEEEEEEEEYYYYYYYYYYYYYYYYYY ERRRROOOOOOORRRRRRRRRRRRR GRPC', $this->newAccessToken($storeId));
                break;
            }
    ...
    public function deleted_products(int $storeId, int $product_last_update = 1)
    {
        $product_client = ClientsBuilder::getProductsClient();
        ...
        if ($this->checkGrpcErrors($prod_arr_api, $storeId) == false) {
            return;
        }
    ...
    public function getProduct(string $product_uuid, int $storeId)
    {
        $product_client = ClientsBuilder::getProductsClient();
        ...
        if ($this->checkGrpcErrors($res_arr, $storeId) == false) {
            return false;
        }
